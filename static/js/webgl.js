...
WebGLPlayer.prototype.renderFrame = function (frame_data, width, height) {
    if (!this.gl) {
    ...
    var gl = this.gl;
	var u_offset = width * height;
	var u_size = (width * height) >> 2;
	var v_offset = u_offset + u_size;
	var v_size = u_size;
    gl.viewport(0, 0, gl.canvas.width, gl.canvas.height);
    gl.clearColor(0.0, 0.0, 0.0, 0.0);
    gl.clear(gl.COLOR_BUFFER_BIT);

    gl.y.fill(width, height, frame_data.subarray(0, u_offset));
    gl.u.fill(width >> 1, height >> 1, frame_data.subarray(u_offset, u_offset + u_size));
    gl.v.fill(width >> 1, height >> 1, frame_data.subarray(v_offset, v_offset + v_size));
